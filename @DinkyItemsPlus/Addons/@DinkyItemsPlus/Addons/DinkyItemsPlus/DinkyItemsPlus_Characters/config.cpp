class CfgPatches
{
	class DinkyItemsPlus_Characters
	{
		units[] = {"TortillaBag","CourierBag","FurCourierBag","ImprovisedBag","FurImprovisedBag","DryBag_ColorBase","HuntingBag","MountainBag_ColorBase","SmershBag","ChildBag_ColorBase","GhillieBushrag_ColorBase","GhillieTop_ColorBase","GhillieSuit_ColorBase","LeatherSack_ColorBase","AssaultBag_ColorBase","CoyoteBag_ColorBase","AliceBag_ColorBase","LeatherBelt_ColorBase","SportGlasses_ColorBase","AviatorGlasses","DesignerGlasses","ThickFramesGlasses","ThinFramesGlasses","TacticalGoggles","NVGHeadstrap","WorkingGloves_ColorBase","TacticalGloves_ColorBase","NBCGloves_ColorBase","SurgicalGloves_ColorBase","LeatherGloves_ColorBase","OMNOGloves_ColorBase","BallisticHelmet_ColorBase","BaseballCap_ColorBase","ZSh3PilotHelmet","ConstructionHelmet_ColorBase","BurlapSackCover","BeanieHat_ColorBase","Ushanka_ColorBase","PumpkinHelmet","MotoHelmet_ColorBase","DarkMotoHelmet_ColorBase","BandanaHead_ColorBase","RadarCap_ColorBase","MilitaryBeret_ColorBase","FlatCap_ColorBase","ZmijovkaCap_ColorBase","CowboyHat_ColorBase","BoonieHat_ColorBase","OfficerHat","PoliceCap","PilotkaCap","TankerHelmet","GorkaHelmet","FirefightersHelmet_ColorBase","SkateHelmet_ColorBase","WeldingMask","PrisonerCap","GhillieHood_ColorBase","LeatherHat_ColorBase","MedicalScrubsHat_ColorBase","GreatHelm","Ssh68Helmet","DirtBikeHelmet_ColorBase","DirtBikeHelmet_Visor","DirtBikeHelmet_Mouthguard","NBCHoodBase","HockeyHelmet_ColorBase","SantasHat","Mich201Helmet","Mich2001Helmet","Universal_Bandana_Redpattern","GasMask","BandanaMask_ColorBase","NioshFaceMask","BalaclavaMask_ColorBase","GP5GasMask","Balaclava3Holes_ColorBase","SantasBeard","MouthRag","SurgicalMask","HockeyMask","Jeans_ColorBase","CargoPants_ColorBase","TTSKOPants","HunterPants_ColorBase","CanvasPants_ColorBase","CanvasPantsMidi_ColorBase","TrackSuitPants_ColorBase","GorkaPants_ColorBase","PolicePants","PolicePantsOrel","ParamedicPants_ColorBase","FirefightersPants_ColorBase","PrisonUniformPants","LeatherPants_ColorBase","MedicalScrubsPants_ColorBase","USMCPants_ColorBase","SlacksPants_ColorBase","BDUPants","NBCPantsBase","Breeches_ColorBase","ShortJeans_ColorBase","Skirt_ColorBase","JumpsuitPants_ColorBase","AthleticShoes_ColorBase","HikingBoots_ColorBase","HikingBootsLow_ColorBase","Wellies_ColorBase","WorkingBoots_ColorBase","JungleBoots_ColorBase","DressShoes_ColorBase","MilitaryBoots_ColorBase","CombatBoots_ColorBase","JoggingShoes_ColorBase","LeatherShoes_ColorBase","Sneakers_ColorBase","NBCBootsBase","Ballerinas_ColorBase","TTSKOBoots","Armband_ColorBase","Sweater_ColorBase","Shirt_ColorBase","TShirt_ColorBase","Hoodie_ColorBase","TacticalShirt_ColorBase","HikingJacket_ColorBase","Raincoat_ColorBase","M65Jacket_ColorBase","TTsKOJacket_ColorBase","GorkaEJacket_ColorBase","RidersJacket_ColorBase","WoolCoat_ColorBase","TrackSuitJacket_ColorBase","PoliceJacket","PoliceJacketOrel","ParamedicJacket_ColorBase","FirefighterJacket_ColorBase","PrisonUniformJacket","MiniDress_ColorBase","QuiltedJacket_ColorBase","BomberJacket_ColorBase","LeatherJacket_ColorBase","HuntingJacket_ColorBase","MedicalScrubsShirt_ColorBase","LabCoat","NurseDress_ColorBase","USMCJacket_ColorBase","Blouse_ColorBase","NBCJacketBase","DenimJacket","TelnyashkaShirt","ChernarusSportShirt","JumpsuitJacket_ColorBase","BDUJacket","ManSuit_ColorBase","WomanSuit_ColorBase","LeatherShirt_ColorBase","PlateCarrierVest","SmershVest","PressVest_ColorBase","UKAssVest_ColorBase","PoliceVest","PlateCarrierHolster","ChestHolster","HighCapacityVest_ColorBase","LeatherStorageVest_ColorBase","HuntingVest","ReflexVest"};
		weapons[] = {};
		requiredVersion = 0.1;
		requiredAddons[] = {"DZ_Characters"};
	};
};
class cfgVehicles
{
	class Clothing;
	class TaloonBag_ColorBase: Clothing
	{
		itemSize[] = {4,5};
	};
	class TortillaBag: Clothing
	{
		itemSize[] = {5,6};
	};
	class CourierBag: Clothing
	{
		itemSize[] = {4,4};
	};
	class FurCourierBag: Clothing
	{
		itemSize[] = {4,4};
	};
	class ImprovisedBag: Clothing
	{
		itemSize[] = {4,5};
	};
	class FurImprovisedBag: Clothing
	{
		itemSize[] = {4,5};
	};
	class DryBag_ColorBase: Clothing
	{
		itemSize[] = {4,5};
	};
	class HuntingBag: Clothing
	{
		itemSize[] = {5,4};
	};
	class MountainBag_ColorBase: Clothing
	{
		itemSize[] = {5,4};
	};
	class SmershBag: Clothing
	{
		itemSize[] = {4,4};
	};
	class ChildBag_ColorBase: Clothing
	{
		itemSize[] = {4,4};
	};
	class GhillieBushrag_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class GhillieTop_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class GhillieSuit_ColorBase: Clothing
	{
		inventorySlot = "Belt";
		itemInfo[] = {"Clothing","Belt"};
		itemSize[] = {1,1};
	};
	class LeatherSack_ColorBase: Clothing
	{
		itemSize[] = {4,4};
	};
	class AssaultBag_ColorBase: Clothing
	{
		itemSize[] = {4,5};
	};
	class CoyoteBag_ColorBase: Clothing
	{
		itemSize[] = {5,4};
	};
	class AliceBag_ColorBase: Clothing
	{
		itemSize[] = {5,4};
	};
	class LeatherBelt_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class SportGlasses_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class AviatorGlasses: Clothing
	{
		itemSize[] = {1,1};
	};
	class DesignerGlasses: Clothing
	{
		itemSize[] = {1,1};
	};
	class ThickFramesGlasses: Clothing
	{
		itemSize[] = {1,1};
	};
	class ThinFramesGlasses: Clothing
	{
		itemSize[] = {1,1};
	};
	class TacticalGoggles: Clothing
	{
		itemSize[] = {1,1};
	};
	class NVGHeadstrap: Clothing
	{
		itemSize[] = {1,1};
	};
	class WorkingGloves_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TacticalGloves_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class NBCGloves_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class SurgicalGloves_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class LeatherGloves_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class OMNOGloves_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class BallisticHelmet_ColorBase: Clothing
	{
		itemSize[] = {2,1};
	};
	class BaseballCap_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class ZSh3PilotHelmet: Clothing
	{
		itemSize[] = {2,1};
	};
	class ConstructionHelmet_ColorBase: Clothing
	{
		itemSize[] = {2,1};
	};
	class BurlapSackCover: Clothing
	{
		itemSize[] = {1,1};
	};
	class BeanieHat_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Ushanka_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class PumpkinHelmet: Clothing
	{
		itemSize[] = {1,1};
	};
	class MotoHelmet_ColorBase: Clothing
	{
		itemSize[] = {2,1};
	};
	class DarkMotoHelmet_ColorBase: Clothing
	{
		itemSize[] = {2,1};
	};
	class BandanaHead_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class RadarCap_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class MilitaryBeret_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class FlatCap_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class ZmijovkaCap_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class CowboyHat_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class BoonieHat_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class OfficerHat: Clothing
	{
		itemSize[] = {1,1};
	};
	class PoliceCap: Clothing
	{
		itemSize[] = {1,1};
	};
	class PilotkaCap: Clothing
	{
		itemSize[] = {1,1};
	};
	class TankerHelmet: Clothing
	{
		itemSize[] = {1,1};
	};
	class GorkaHelmet: Clothing
	{
		itemSize[] = {2,2};
	};
	class FirefightersHelmet_ColorBase: Clothing
	{
		itemSize[] = {2,1};
	};
	class SkateHelmet_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class WeldingMask: Clothing
	{
		itemSize[] = {2,1};
	};
	class PrisonerCap: Clothing
	{
		itemSize[] = {1,1};
	};
	class GhillieHood_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class LeatherHat_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class MedicalScrubsHat_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class GreatHelm: Clothing
	{
		itemSize[] = {2,2};
	};
	class Ssh68Helmet: Clothing
	{
		itemSize[] = {2,1};
	};
	class DirtBikeHelmet_ColorBase: Clothing
	{
		itemSize[] = {2,1};
	};
	class DirtBikeHelmet_Visor: Clothing
	{
		itemSize[] = {2,1};
	};
	class DirtBikeHelmet_Mouthguard: Clothing
	{
		itemSize[] = {2,1};
	};
	class NBCHoodBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class HockeyHelmet_ColorBase: Clothing
	{
		itemSize[] = {2,1};
	};
	class SantasHat: Clothing
	{
		itemSize[] = {1,1};
	};
	class Mich201Helmet: Clothing
	{
		itemSize[] = {2,1};
	};
	class Mich2001Helmet: Clothing
	{
		itemSize[] = {2,1};
	};
	class Universal_Bandana_Redpattern: Clothing
	{
		itemSize[] = {1,1};
	};
	class GasMask: Clothing
	{
		itemSize[] = {2,2};
	};
	class BandanaMask_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class NioshFaceMask: Clothing
	{
		itemSize[] = {1,1};
	};
	class BalaclavaMask_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class GP5GasMask: Clothing
	{
		itemSize[] = {2,2};
	};
	class Balaclava3Holes_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class SantasBeard: Clothing
	{
		itemSize[] = {1,1};
	};
	class MouthRag: Clothing
	{
		itemSize[] = {1,1};
	};
	class SurgicalMask: Clothing
	{
		itemSize[] = {1,1};
	};
	class HockeyMask: Clothing
	{
		itemSize[] = {1,1};
	};
	class Jeans_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class CargoPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TTSKOPants: Clothing
	{
		itemSize[] = {1,1};
	};
	class HunterPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class CanvasPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class CanvasPantsMidi_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TrackSuitPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class GorkaPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class PolicePants: Clothing
	{
		itemSize[] = {1,1};
	};
	class PolicePantsOrel: Clothing
	{
		itemSize[] = {1,1};
	};
	class ParamedicPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class FirefightersPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class PrisonUniformPants: Clothing
	{
		itemSize[] = {1,1};
	};
	class LeatherPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class MedicalScrubsPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class USMCPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class SlacksPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class BDUPants: Clothing
	{
		itemSize[] = {1,1};
	};
	class NBCPantsBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Breeches_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class ShortJeans_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Skirt_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class JumpsuitPants_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class AthleticShoes_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class HikingBoots_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class HikingBootsLow_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Wellies_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class WorkingBoots_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class JungleBoots_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class DressShoes_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class MilitaryBoots_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class CombatBoots_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class JoggingShoes_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class LeatherShoes_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Sneakers_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class NBCBootsBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Ballerinas_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TTSKOBoots: Clothing
	{
		itemSize[] = {1,1};
	};
	class Armband_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Sweater_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Shirt_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TShirt_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Hoodie_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TacticalShirt_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class HikingJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Raincoat_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class M65Jacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TTsKOJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class GorkaEJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class RidersJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class WoolCoat_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class TrackSuitJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class PoliceJacket: Clothing
	{
		itemSize[] = {1,1};
	};
	class PoliceJacketOrel: Clothing
	{
		itemSize[] = {1,1};
	};
	class ParamedicJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class FirefighterJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class PrisonUniformJacket: Clothing
	{
		itemSize[] = {1,1};
	};
	class MiniDress_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class QuiltedJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class BomberJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class LeatherJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class HuntingJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class MedicalScrubsShirt_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class LabCoat: Clothing
	{
		itemSize[] = {1,1};
	};
	class NurseDress_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class USMCJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class Blouse_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class NBCJacketBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class DenimJacket: Clothing
	{
		itemSize[] = {1,1};
	};
	class TelnyashkaShirt: Clothing
	{
		itemSize[] = {1,1};
	};
	class ChernarusSportShirt: Clothing
	{
		itemSize[] = {1,1};
	};
	class JumpsuitJacket_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class BDUJacket: Clothing
	{
		itemSize[] = {1,1};
	};
	class ManSuit_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class WomanSuit_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class LeatherShirt_ColorBase: Clothing
	{
		itemSize[] = {1,1};
	};
	class PlateCarrierVest: Clothing
	{
		itemSize[] = {2,2};
	};
	class SmershVest: Clothing
	{
		itemSize[] = {4,4};
	};
	class PressVest_ColorBase: Clothing
	{
		itemSize[] = {2,2};
	};
	class UKAssVest_ColorBase: Clothing
	{
		itemSize[] = {2,2};
	};
	class PoliceVest: Clothing
	{
		itemSize[] = {2,2};
	};
	class PlateCarrierHolster: Clothing
	{
		itemSize[] = {1,1};
	};
	class ChestHolster: Clothing
	{
		itemSize[] = {1,1};
	};
	class HighCapacityVest_ColorBase: Clothing
	{
		itemSize[] = {2,2};
	};
	class LeatherStorageVest_ColorBase: Clothing
	{
		itemSize[] = {2,2};
	};
	class HuntingVest: Clothing
	{
		itemSize[] = {2,2};
	};
	class ReflexVest: Clothing
	{
		itemSize[] = {2,2};
	};
};
